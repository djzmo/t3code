...
| V1.5      | Staging, payload validation, smoke harnesses, and Windows packaging are implemented. Windows primary uses stdin/`--bootstrap-fd 0`. Packaged debug smokes (normal + `--kill-host`) passed locally on 2026-08-20 and in CI on SHA `5e196df77` (run `32451938005`) for windows-2022, macos-14, and ubuntu-22.04 xvfb AppImage.                                       | Wayland AppImage launch and attended core-UI matrix remain pending.                                                                                                                   |
| V1.6      | Debug and release artifact jobs passed on ubuntu-22.04, windows-2022, and macos-14 for SHA `5e196df77` (run `32451938005`), including Windows packaged smoke.                                                                                                                                                                                                      | Three-webview UI evidence and LAN/Tailscale web/mobile remote-readiness remain pending.                                                                                               |
...
External Tauri CI on SHA `5e196df77` (run `32451938005`, 2026-08-21)
built debug and release artifacts on ubuntu-22.04, windows-2022, and macos-14.
Ubuntu, macOS, and Windows debug jobs logged both `Tauri smoke passed (normal)`
and `Tauri smoke passed (forced-kill)`. Windows smoke uses
`target/debug/agent-nanoni-desktop.exe` with stdin/`--bootstrap-fd 0`. The
harness expands `TEMP` so libuv does not abort on the runner's 8.3 short path.
Repo `CI` (`Check`/`Test`) and `Mobile Fingerprint Check` remain queued on
Blacksmith runners at documentation time and are not treated as Tauri smoke
failures.
...
| --------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| WKWebView | smoke-ci | macos-14 packaged debug smoke completed on SHA `5e196df77` (run `32451938005`). Core-UI matrix not filled.                                          |
| WebKitGTK | smoke-ci | ubuntu-22.04 xvfb AppImage smoke reached `first-roundtrip` on SHA `5e196df77`. Wayland launch still pending.                                        |
| WebView2  | smoke-ci | windows-2022 packaged debug smoke logged `Tauri smoke passed (normal)` and `(forced-kill)` on SHA `5e196df77` (run `32451938005`). Core-UI pending. |
